...
| `vaultwarden.nix` | Vaultwarden password manager on :8222 (Bitwarden-compatible, sqlite); `SIGNUPS_ALLOWED = true` for first login — flip to `false` after creating the account; nightly 23:00 sqlite backup to `/var/backup/vaultwarden`; admin token from sops secret `admin-token` (`secrets/vaultwarden.yaml`) rendered into an env template; imported by `mactoncino` only |
...
- **Hosts:**
  ...
  - `mactoncino` — headless server (`nixosModules.base` + `media` + `arr` + `slskd` + `pihole` + `home-assistant` + `glance` + `homepage` + `n8n`), systemd-boot with `boot.loader.timeout = 0` (no menu; hold Space to enter systemd-boot), tailscale; media under `/srv/media`. Ports: 53 DNS (Pi-hole), 80/443 Caddy (public: music.mactonet.com → Navidrome, paperless.mactonet.com → Paperless), 3000 Pi-hole UI, 4533 Navidrome, 5030 slskd WebUI, 50300 Soulseek listen, 5678 n8n, 631 CUPS web UI, 6881 qBittorrent torrent, 7878 Radarr, 8080 Glance, 8082 Homepage, 8081 qBittorrent WebUI, 8096 Jellyfin, 8123 Home Assistant, 8222 Vaultwarden, 8989 Sonarr, 9696 Prowlarr, 28981 Paperless.
