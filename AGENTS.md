...
- **Deploy locally with notifications (`nos`, from `modules/system/rebuild.nix`):** runs `nh os <action>` as a transient `mactoflake-rebuild-<action>.service` systemd user unit created via `systemd-run --user --collect` (survives terminal close, no double-starts) with notify-send on success/failure. Transient on purpose: switch-to-configuration starts/restarts user units defined in the config during activation, which deadlocks or kills an activation hosted in one. `nos` (or `nos switch|boot|test|dry|build`) follows the live output — Ctrl-C detaches, the rebuild keeps running; `nos start [action]` backgrounds it; `nos watch`/`nos status` inspect. Log: `~/.cache/mactoflake/rebuild.log` or `journalctl --user -u 'mactoflake-rebuild-*'`. Hyprland binds: SUPER+SHIFT+R starts, SUPER+ALT+R opens a floating watcher.
...
| `homepage.nix` | Homepage dashboard on :8082 (staging alongside Glance on :8080; will take over :8080 once Glance is removed); declarative `services.homepage-dashboard.{settings,widgets,services,bookmarks,customCSS}`; live API-key service integrations (Jellyfin, Navidrome, Sonarr/Radarr/Prowlarr, qBittorrent, slskd, Pi-hole v6, Home Assistant, Paperless-ngx) via sops template `homepage-env` → `environmentFiles` (`{{HOMEPAGE_VAR_*}}` substitution, values from `secrets/homepage.yaml` + reused `slskd.yaml`/`paperless.yaml` secrets); `siteMonitor` for services without widgets (n8n, Router, Vaultwarden — homepage has no vaultwarden widget type); imported by `mactoncino` only |
...
| `minecraft.nix` | `mactoflake.minecraft.servers` attrset (enable/slug/memory/version/port/whitelist/allowFlight/aikarFlags/autoStart/viewDistance/simulationDistance/difficulty/motd/pauseWhenEmptySeconds/jvmXXOpts/extraEnv/restartCalendar) of modpack servers run as `itzg/minecraft-server:java21` docker containers; `pauseWhenEmptySeconds` sets `PAUSE_WHEN_EMPTY_SECONDS` (-1 disables pausing when empty); `extraEnv` merges extra container env vars; `restartCalendar` (systemd OnCalendar) restarts the server weekly-ish but only if currently running; `jvmXXOpts` + `aikarFlags = true` with a GC selector triggers an eval warning (`AUTO_CURSEFORGE`, pack auto-installs at first start, `version` pins via `CF_FILENAME_MATCHER`, no CF API key needed — bundled in the image); state under `/srv/minecraft/<name>` (user/group `minecraft`, uid/gid 996); `--network=host` + `SERVER_PORT` so the centralized firewall governs access; ports must be unique across enabled servers (asserted); requires rooted docker (`mactoflake.containers` with `rootless = false`); imported by `mactoncino` only (atm10 on :25565 enabled, atm11 parked on :25566) |
| `minecraft.nix` | `mactoflake.minecraft.servers` attrset (enable/slug/memory/version/port/whitelist/allowFlight/aikarFlags/autoStart/viewDistance/simulationDistance/difficulty/motd/pauseWhenEmptySeconds/jvmXXOpts) of modpack servers run as `itzg/minecraft-server:java21` docker containers; `pauseWhenEmptySeconds` sets `PAUSE_WHEN_EMPTY_SECONDS` (-1 disables pausing when empty); `jvmXXOpts` + `aikarFlags = true` with a GC selector triggers an eval warning (`AUTO_CURSEFORGE`, pack auto-installs at first start, `version` pins via `CF_FILENAME_MATCHER`, no CF API key needed — bundled in the image); state under `/srv/minecraft/<name>` (user/group `minecraft`, uid/gid 996); `--network=host` + `SERVER_PORT` so the centralized firewall governs access; ports must be unique across enabled servers (asserted); requires rooted docker (`mactoflake.containers` with `rootless = false`); also contributes a `Gaming` service group (gamedig status widgets querying `127.0.0.1:<port>` over UDP) to `services.homepage-dashboard` when homepage is enabled; imported by `mactoncino` only (atm10 on :25565 enabled, atm11 parked on :25566) |
