...
| `droppedneedle.nix` | DroppedNeedle (music request/library engine that drives slskd, :8688) as a Docker OCI container, image pinned by digest; mounts `/srv/media` → `/data` (library + slskd downloads on one fs → atomic imports); state in `/var/lib/droppedneedle/{config,cache,plugins}`; enables rooted Docker on the host (server-side, separate from the desktop `containers` module); imported by `mactoncino` only |
