...
- **Hosts:**
  ...
  - `mactoncino` — headless server (`nixosModules.base` + `media` + `arr` + `slskd` + `pihole` + `home-assistant` + `glance` + `homepage` + `n8n`), systemd-boot with `boot.loader.timeout = 0` (no menu; hold Space to enter systemd-boot), tailscale; media under `/srv/media`. Ports: 53 DNS (Pi-hole), 80/443 Caddy (public: music.mactonet.com → Navidrome, paperless.mactonet.com → Paperless, jelly.mactonet.com → Jellyfin, vault.mactonet.com → Vaultwarden), 3000 Pi-hole UI, 4533 Navidrome, 5030 slskd WebUI, 50300 Soulseek listen, 5678 n8n, 631 CUPS web UI, 6881 qBittorrent torrent, 7878 Radarr, 8080 Glance, 8082 Homepage, 8081 qBittorrent WebUI, 8096 Jellyfin, 8123 Home Assistant, 8222 Vaultwarden, 8989 Sonarr, 9696 Prowlarr, 28981 Paperless.
